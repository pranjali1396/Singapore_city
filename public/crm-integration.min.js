...
(function () {
  ...
  if (window.__CRM_SCRIPT_INITIALIZED__) {
    console.log('[CRM] Script already initialized, skipping...');
    return;
  }
  window.__CRM_SCRIPT_INITIALIZED__ = true;
  ...
  function init() {
    ...
    async function handleFormSubmit(e, form) {
      try {
        ...
        const now = Date.now();
        const lastSubmit = form.dataset.lastSubmit ? parseInt(form.dataset.lastSubmit) : 0;

        if (now - lastSubmit < 3000) {
          log('⚠️ Duplicate submission prevented (within 3 seconds)');
          return;
        }
